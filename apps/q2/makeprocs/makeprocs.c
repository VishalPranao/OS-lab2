...
void main(int argc, char *argv[]) {
  ...
  buffer->s_fullslots = sem_create(0);
  /* Keep one slot unused, matching Q1's circular-buffer convention. */
  buffer->s_emptyslots = sem_create(BUFFER_SIZE - 1);
  if (buffer->s_fullslots == SYNC_FAIL ||
      buffer->s_emptyslots == SYNC_FAIL) {
    Printf("ERROR: slot semaphore creation failed\n");
    Exit();
  }
