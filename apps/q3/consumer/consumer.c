...
void main(int argc, char *argv[]) {
  ...
  while (next < NUM_CHARACTERS) {
    ...
    while (buffer->head == buffer->tail ||
      ...
      if (cond_wait(buffer->digit_ready[next]) != SYNC_SUCCESS) {
        Printf("ERROR: digit wait failed\n");
        Exit();
      }
    }
    ...
    next++;
    if (cond_signal(buffer->not_full) != SYNC_SUCCESS) {
      Printf("ERROR: not_full signal failed\n");
      Exit();
    }
